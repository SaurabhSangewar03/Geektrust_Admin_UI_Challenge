...
const User = (props) => {
  ...
  return (
    <tr key={user.id} className={user.selected ? styles.selected : styles.deselected}>
      <td>
      ...
      <td>
        ...
        />
        ) : (<span className={user.role === 'admin' ? styles.admin : styles.member}>
        <input
          className={user.edit ? styles.editable : styles.readOnly}
          readOnly={!user.edit}
          type="text"
      ...
      <td className={styles.icons}>
        ...
        <i onClick={() => deleteUser(user.id)} className="fas fa-trash-alt delete"></i>
      </td>
    </tr>
  );
};
